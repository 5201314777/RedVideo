...
class NET_DVR_JPEGPARA(Structure):
    _fields_ = [
        ("wPicSize", C_WORD),     # 图片尺寸，0-默认尺寸(与当前码流分辨率)
        ("wPicQuality", C_WORD)   # 图片质量系数，0-最好，1-较好，2-一般
    ]
...
class NET_DVR_DEVICEINFO_V40(Structure):
    _fields_ = [
        ...
        ('iResidualValidity', C_LONG),  # 该用户密码剩余有效天数，单位：天，返回负值，表示密码已经超期使用，例如"-"表示密码已经超期使用3天"
...
class NET_DVR_CHANNEL_CFG(Structure):
    _fields_ = [
        ("byChanNum", c_byte * 64),  # 通道启用状态数组
        ("byRes", c_byte * 64)
    ]

# 常量定义
NET_DVR_GET_CHANNEL_CFG = 2  # 获取通道配置的命令码
...
class NET_DVR_IPDEVINFO_V31(Structure):
    _fields_ = [
        ...
        ("byProType", c_byte),               # 协议类型
        ("byRes1", c_byte * 2),
        ("struIP", c_char * 16),             # IP地址
        ("wDVRPort", c_uint16),              # 端口
        ("sUserName", c_char * 64),           # 用户名
        ("sPassword", c_char * 64),          # 密码
...
class NET_DVR_IPPARACFG_V40(Structure):
    _fields_ = [
        ...
        ("dwGroupNum", c_uint32),            # 组数
        ("dwAChanNum", c_uint32),           # 模拟通道数
        ("dwDChanNum", c_uint32),           # 数字通道数（IP通道数）
        ("struIPDevInfo", NET_DVR_IPDEVINFO_V31 * 64),  # IP设备信息数组
        ("byRes", c_byte * 512)              # 保留字段
    ]
##############################################################################################################
...
#定义了一个指向NET_DVR_USER_LOGIN_INFO结构体的指针类型
...
#定义了一个指向NET_DVR_LOCAL_SDK_PATH结构体的指针类型
...
#定义了一个指向NET_DVR_ALARMER结构体的指针类型
...
#定义了一个指向NET_DVR_SETUPALARM_PARAM结构体的指针类型
